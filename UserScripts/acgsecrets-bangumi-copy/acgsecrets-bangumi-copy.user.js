  ...
  function extractNames() {
    ...
    workNodes.forEach((node) => {
      ...
      if (isContinuing && (hasTaggedSequel || isNameSequelOrArc)) {
        ...
        categorized = true;
      } else if (isContinuing && hasTaggedNewSeries) {
        // 2. 跨季繼續播放 且 為新作
        continuingAndNew.push(name);
        categorized = true;
      } else if (hasParentalGuidance && (hasTaggedSequel || isNameSequelOrArc)) {
        // 3. 家長指引 且 符合續作判斷
        parentalAndSequel.push(name);
        categorized = true;
      } else if (hasParentalGuidance && hasTaggedNewSeries) {
        // 4. 家長指引 且 為新作
        parentalAndNew.push(name);
        categorized = true;
      } else if (hasTaggedSequel) {
        // 5. 單純標籤續作 (未被複合條件捕獲)
        taggedSequels.push(name);
        categorized = true;
      } else if (hasTaggedNewSeries) {
        // 6. 單純標籤新作 (未被複合條件捕獲)
        taggedNewSeries.push(name);
        categorized = true;
      } else if (isNameSequelOrArc) {
        // 7. 無標籤但依名稱判斷為續作/特別篇
