...
(function () {
    ...
    window.alert = function (message) {
      try {
        if (message === errorCartFull) {
          console.warn(
            '[Main Context] Cart full detected. Switching to automatic checkout mode...',
          );
  ...
  async function startCheckoutProcess() {
    ...
    try {
      ...
      const PURCHASE_BUTTON_SELECTOR =
        '.a-mu-large-r-btn--primary.forward[data-action-label="無料でもらう"]';
      ...
      if (totalPrice === '0') {
        if ($purchaseButton.length > 0) {
          ...
          console.log(
            '[Checkout Process] Price is 0. Clicking purchase and letting the site navigate.',
          );
        ...
        alert(
          `❌ 購物車中有非免費商品 (總價：${totalPrice} 日圓)。請手動處理後，點擊批量加入按鈕繼續。`,
        );
        localStorage.removeItem(CHECKOUT_MODE_KEY);
      }
    } catch (e) {
  ...
  async function startBatchProcess() {
    ...
    try {
      ...
      alert(
        '❌ 批量加入購物車流程發生嚴重錯誤，已停止自動化。請檢查控制台 (Console) 以獲取更多資訊。',
      );
    }
  }
  ...
  function handleNextPage(current_page_added_count) {
    ...
    if (isNextPageValid) {
      ...
      console.log(
        `✅ 流程已全部完成！(本頁加入 ${current_page_added_count} 本)。正在返回 BookWalker 首頁。`,
      );
