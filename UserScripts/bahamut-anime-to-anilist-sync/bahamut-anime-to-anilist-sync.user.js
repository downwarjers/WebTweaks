...
(function () {
    ...
    const _ = {
        ...
        on: (el, events, handler) => events.split(' ').forEach(evt => el && el.addEventListener(evt, handler)),
        html: (str) => {
    ...
    // ================= [Utils] 工具函式與 Logger =================
    const Log = {
        info: (...args) => CONSTANTS.DEBUG && console.log("%c[AniList]", "color:#3db4f2;font-weight:bold;", ...args),
        warn: (...args) => CONSTANTS.DEBUG && console.warn("%c[AniList]", "color:#ffca28;font-weight:bold;", ...args),
        error: (...args) => console.error("%c[AniList Error]", "color:#ff5252;font-weight:bold;", ...args)
    };
    ...
    const AniListAPI = {
        ...
        async request(query, variables) {
            ...
            Log.info("API Request:", { query: query.substr(0, 50) + "...", variables });
    ...
    const UI = {
        ...
        initNavbar(nav) {
            ...
            const li = _.html(`<li class="al-nav-item"><a class="al-nav-link" id="al-trigger"><span id="al-icon">⚪</span><span id="al-text">AniList</span><span id="al-user-status" class="al-user-status"></span><span id="al-title" class="al-nav-title" style="display:none;"></span></a></li>`);
            nav.appendChild(li);
            
            _.$("#al-trigger").addEventListener("click", () => this.openModal());

            const modal = _.html(`<div id="al-modal" class="al-modal-overlay"><div class="al-modal-content"><div class="al-modal-header"><strong>AniList 設定</strong><button class="al-close-btn">&times;</button></div><div class="al-modal-body" id="al-modal-body"></div></div></div>`);
            document.body.appendChild(modal);

            _.$(".al-close-btn", modal).addEventListener("click", () => _.fadeOut(modal));
            modal.addEventListener("click", (e) => { if (e.target.id === "al-modal") _.fadeOut(modal); });
        },
        updateNav(type, msg) {
            const $icon = _.$("#al-icon"), $text = _.$("#al-text"), $title = _.$("#al-title"), $uStatus = _.$("#al-user-status");
            ...
            if (type === CONSTANTS.STATUS.DONE) {
                this.statusTimer = setTimeout(() => {
                    $icon.textContent = "✅"; $text.textContent = "已連動";
        ...
        renderSettings(container) {
            ...
            const toggleCustom = () => { _.$("#custom-sec-group", container).style.display = _.$("#set-mode", container).value === "custom" ? "flex" : "none"; };
            toggleCustom();
        ...
        async renderHomeBound(container) {
            ...
            try {
                ...
                _.$("#home-status", container).addEventListener("change", async function () {
                    ...
                    } catch (e) { Utils.showToast("❌ 更新失敗: " + e.message); this.disabled = false; }
                });
        ...
        renderHomeUnbound(container) {
            ...
            if (App.state.candidate) {
                _.$("#btn-quick", container).addEventListener("click", () => App.bindSeries(App.state.candidate.id, App.state.candidate.title.native));
            }
            ...
            const doSearch = async () => {
                ...
                try {
                    ...
                    _.$$(".bind-it", resContainer).forEach(btn => {
                        btn.addEventListener("click", function() { App.bindSeries(this.dataset.id, this.dataset.title); });
                    });
                } catch (e) { resContainer.innerHTML = `<div style="color:red;">Error: ${e.message}</div>`; }
            };

            _.$("#btn-search", container).addEventListener("click", doSearch);
            _.$("#search-in", container).addEventListener("keypress", (e) => { if (e.key === "Enter") doSearch(); });
        ...
        async renderSeries(container) {
            ...
            if (!baseId) { container.innerHTML = '<div style="padding:20px;text-align:center;color:#999;">請先在主頁綁定作品</div>'; return; }
            try {
                ...
                _.$("#save-series", container).addEventListener("click", () => {
                    ...
                    _.$$(".series-row", container).forEach(row => {
                        ...
                        if (cb.checked && val) {
                            newRules.push({ start: val, id: parseInt(row.dataset.id), title: row.dataset.title });
                        }
                    });
            ...
            } catch (e) { container.innerHTML = `<div style="padding:20px;color:red;">載入失敗: ${e.message}</div>`; }
        }
    };
    ...
    const App = {
        ...
        getAcgLink() {
            ...
            const alt = [...document.querySelectorAll(CONSTANTS.SELECTORS.acgLinkAlt)].find(a => a.textContent.includes("作品資料"));
        ...
        async determineActiveRule() {
            ...
            if (this.state.activeRule && this.state.token) {
                try {
                    ...
                    this.updateUIStatus();
                } catch (e) { Log.error("Fetch status error:", e); }
            }
        },
        startVideoHunt() {
        ...
        async tryAutoBind() {
            ...
            for (let term of terms) {
                ...
                } catch (e) { Log.warn("AutoBind Search Error:", e); }
                if (match) break;
            }
            if (!match && site) {
                ...
                if (range) {
                    ...
                    } catch (e) { Log.warn("AutoBind Range Error:", e); }
                }
            }
            ...
            try {
                ...
                chain.forEach(m => {
                    if (m.id === parseInt(id) || m.suggestedStart <= maxPageEp) {
                        newRules.push({ start: m.suggestedStart, id: m.id, title: m.title.native || m.title.romaji });
                    }
                });
            } catch (e) { Log.warn("Series Bind Failed:", e); }

            if (newRules.length === 0) newRules.push({ start: 1, id: parseInt(id), title: title });
            else {
                const uniqueRules = [];
                const seenIds = new Set();
                newRules.forEach(r => { if(!seenIds.has(r.id)){ seenIds.add(r.id); uniqueRules.push(r); } });
        ...
        async fetchBahaData(url) {
            try {
                ...
                const dateJpStr = listItems.find(el => el.textContent.includes("當地"))?.textContent.split("：")[1];
                const dateTwStr = listItems.find(el => el.textContent.includes("台灣"))?.textContent.split("：")[1];
                
                let siteDomain = "";
                const offLinkEl = [...doc.querySelectorAll('.ACG-box1listB > li')].find(el => el.textContent.includes("官方網站"))?.querySelector('a');
                if (offLinkEl) {
                    try {
                        const u = new URL(offLinkEl.href, "https://acg.gamer.com.tw");
                        siteDomain = Utils.extractDomain(u.searchParams.get('url') || offLinkEl.href);
                    } catch (e) { Log.error(e); }
                }
            ...
            } catch (e) { Log.error("Baha Data Error", e); return null; }
        },
        updateUIStatus() {
