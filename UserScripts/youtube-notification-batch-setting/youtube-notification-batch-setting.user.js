    ...
    try {
      ...
      while (isRunning) {
        ...
        const channels = document.querySelectorAll(
          '#grid-container ytd-channel-renderer:not([data-batch-processed="true"])',
        );
        globalStats.totalFound = document.querySelectorAll(
          '#grid-container ytd-channel-renderer',
        ).length;
            ...
            if (currentLabel.includes(targetMode.ariaLabelCheck)) {
              ...
              try {
                ...
                if (menuItem) {
                  ...
                  console.error(
                    `[${channelName}] -> 找不到「${targetMode.menuText}」選項！腳本可能已失效。`,
                  );
                  globalStats.failed++;
        ...
        const remainingChannels = document.querySelectorAll(
          '#grid-container ytd-channel-renderer:not([data-batch-processed="true"])',
        );
      ...
      updateUIState(false);
      updateStatus(
        `處理完成！ 成功: ${globalStats.processed} | 跳過: ${globalStats.skipped} | 失敗: ${globalStats.failed} (總共 ${globalStats.totalFound})`,
      );
    }
  }
    ...
    return new Promise((resolve) => {
      ...
      const interval = setInterval(() => {
        ...
        if (newHeight > currentHeight) {
          ...
          // 等待中...
          updateStatus(
            `滾動頁面... 等待新內容... (${Math.floor(elapsedTime / 1000)}s / ${timeout / 1000}s)`,
          );
        }
      }, 500); // 每 500ms 檢查一次
    });
