...
(function() {
    ...
    const CONFIG = {
        ...
        DATA: {
            ...
            DISTRICTS: [
                ...
                "東區","北區","香山區","竹北市","湖口鄉","新豐鄉","新埔鎮","關西鎮","芎林鄉","寶山鄉","竹東鎮","五峰鄉","橫山鄉","尖石鄉","北埔鄉","峨眉鄉",
                // 苗栗/台中
                "竹南鎮","頭份市","三灣鄉","南庄鄉","獅潭鄉","後龍鎮","通霄鎮","苑裡鎮","苗栗市","造橋鄉","頭屋鄉","公館鄉","大湖鄉","泰安鄉","銅鑼鄉","三義鄉","西湖鄉","卓蘭鎮",
                "中區","南區","西區","北區","北屯區","西屯區","南屯區","太平區","大里區","霧峰區","烏日區","豐原區","后里區","石岡區","東勢區","和平區","新社區","潭子區","大雅區","神岡區","大肚區","沙鹿區","龍井區","梧棲區","清水區","大甲區","外埔區","大安區",
                // 彰化/南投/雲林
                ...
                "中西區","安平區","安南區","永康區","歸仁區","新化區","左鎮區","玉井區","楠西區","南化區","仁德區","關廟區","龍崎區","官田區","麻豆區","佳里區","西港區","七股區","將軍區","學甲區","北門區","新營區","後壁區","白河區","東山區","六甲區","下營區","柳營區","鹽水區","善化區","大內區","山上區","新市區","安定區",
                // 高雄
    ...
    function injectUI(modal) {
        ...
        const createRow = () => { const div = document.createElement('div'); div.style.display = 'flex'; div.style.alignItems = 'center'; div.style.gap = '10px'; return div; };
        const createLabel = (text) => { const lbl = document.createElement('label'); lbl.innerText = text; lbl.style.fontSize = "13px"; lbl.style.fontWeight = "bold"; lbl.style.color = "#202124"; lbl.style.minWidth = "35px"; return lbl; };
        ...
        cityInput.type = "text"; cityInput.value = initialData.city; cityInput.placeholder = "縣市";
        ...
        districtInput.type = "text"; districtInput.value = initialData.district; districtInput.placeholder = "行政區";
        ...
        row4.appendChild(createLabel("位置:")); row4.appendChild(cityInput); row4.appendChild(districtInput);
            ...
            btn.onclick = async () => {
                ...
                try {
                    ...
                    if (exists) {
                        ...
                        statusMsg.innerText = `資料庫已有此店`; btn.disabled = false;
                    } else {
                        btn.innerText = '寫入中...';
        ...
        panel.appendChild(row1); panel.appendChild(row2); panel.appendChild(row3); panel.appendChild(row4); panel.appendChild(statusMsg); panel.appendChild(btn);
